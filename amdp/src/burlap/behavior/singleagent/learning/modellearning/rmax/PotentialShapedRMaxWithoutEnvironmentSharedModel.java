 ...
 *
 * This is an environment free extension of the original potential based RMax algorithm. It also allows sharing of paramaters with other domains that might
 * use the same base environment.
 * @author Nakul Gopalan
 *
 */
public class PotentialShapedRMaxWithoutEnvironmentSharedModel extends MDPSolver {
     ...
     * The model of the world that is being learned.
     */
    public FactoredTabularModel sharedModel;
...
//     */
//    public PotentialShapedRMaxWithoutEnvironmentSharedModel(SADomain domain, double gamma, HashableStateFactory hashingFactory, double maxReward, int nConfident,
...
//     * @param maxVIPasses the maximum number of VI iterations per replan.
//     */
//    public PotentialShapedRMaxWithoutEnvironmentSharedModel(SADomain domain, double gamma, HashableStateFactory hashingFactory, PotentialFunction potential, int nConfident,
//                                          double maxVIDelta, int maxVIPasses){
//
//        this.solverInit(domain, gamma, hashingFactory);
//        this.model = new RMaxModel(new TabularModel(domain, hashingFactory, nConfident),
...
//    }
     ...
     */
    public PotentialShapedRMaxWithoutEnvironmentSharedModel(SADomain domain, HashableStateFactory hashingFactory, PotentialFunction potential,
                                          KWIKModel model, ModelLearningPlanner plannerGenerator, double gamma){
     ...
     * @param hashingFactory a state hashing factory for indexing states
     * @param potential the admissible potential function
     * @param sharedFM the model/model-learning algorithm to use
     */
    public PotentialShapedRMaxWithoutEnvironmentSharedModel(SADomain domain, HashableStateFactory hashingFactory, PotentialFunction potential,
                                                            FactoredTabularModel sharedFM, int nConfidence, double maxVIDelta, int maxVIPasses, double gamma){

        this.gamma = gamma;
        this.solverInit(sharedFM.getSourceDomain(), gamma, hashingFactory);
        this.sharedModel = sharedFM;

//        FactoredTabularModel sharedFM =  sharedModel;

        FactoredTabularModel localFM = new FactoredTabularModel(sharedFM.getSourceDomain(), sharedFM.getHashingFactory(), sharedFM.getnConfident(),
                                                                sharedFM.getStateNodes(), new HashSet<HashableState>());


        this.model = new RMaxModel(localFM,  potential, gamma, sharedFM.getSourceDomain().getActionTypes());


        this.modelPlanner = new VIModelLearningPlanner(sharedFM.getSourceDomain(), this.model, gamma, hashingFactory, maxVIDelta, maxVIPasses);
        this.modelPlanner.setModel(this.model);
...
//    }
    ...
    public void beginEpisode(State s){
        Episode ea = new Episode(s);

        State initialState = s;

        this.modelPlanner.initializePlannerIn(s);


        currentPolicy = this.createUnmodeledFavoredPolicy();
...
//
//        currentAction = currentPolicy.action(currentState);
//        return currentAction;
//
//    }

    public void updateStateAndModel(State currentState, Action currentAction, State nextState, List<Double> rewards){
        boolean terminate = this.domain.getModel().terminal(nextState);

        double discountedReward = getDiscountedReward(rewards);
        // reward is a discounted reward for that time step
        // model probability can
        EnvironmentOutcome eo = new EnvironmentOutcome(currentState, currentAction, nextState, discountedReward, terminate);
//        currentEpisode.transition(eo);

        // make sure to have the same table
        ((FactoredTabularModel)this.model.getSourceModel()).setStateNodes((this.sharedModel).getStateNodes());


        boolean modeledTerminal = this.model.terminal(eo.op);

        if(!this.model.transitionIsModeled(currentState, currentAction)
                || (!KWIKModel.Helper.stateTransitionsModeled(model, this.getActionTypes(), eo.op) && !modeledTerminal)){
            // update both models
            this.sharedModel.updateModel(eo);
            this.model.updateModel(eo);

            //made sure the transition table is a global table!
//            ((FactoredTabularModel)this.model.getSourceModel()).setStateNodes(((FactoredTabularModel)this.sharedModel.getSourceModel()).getStateNodes());

            if(this.model.transitionIsModeled(currentState, currentAction) || (eo.terminated != modeledTerminal && modeledTerminal != this.model.terminal(eo.op))){
                this.modelPlanner.modelChanged(currentState);
                currentPolicy = this.createUnmodeledFavoredPolicy();
            }
        }
    ...
    private double getDiscountedReward(List<Double> rewards) {
        double sum = 0.;
        for(int i =0;i<rewards.size();i++){
            sum+=gamma*rewards.get(i);
        }
        return sum;
    }

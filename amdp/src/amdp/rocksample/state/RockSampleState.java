...
public class RockSampleState implements MutableOOState{
    ...
    public List<RockSampleMapWall> walls = new ArrayList<RockSampleMapWall>();


    public RockSampleState(List<RockSampleMapWall> walls, List<TaxiPassenger> passengers,
                           List<TaxiLocation> locations, RoverAgent taxi) {
        this.walls = walls;
    ...
    public MutableOOState renameObject(String objectName, String newName) {
        if(objectName.equals(taxi.name())){
            RoverAgent nagent = taxi.copyWithName(newName);
            this.taxi= nagent;
            return this;
        }
        ...
        if(indW == -1) {
            //copy on write
            RockSampleMapWall nWall = this.walls.get(indW).copyWithName(newName);
    ...
    public ObjectInstance object(String objectName) {
        ...
        if(indW != -1) {
            //copy on write
            RockSampleMapWall nWall = this.walls.get(indW).copy();
            return nWall;
        }
    ...
    public List<ObjectInstance> objectsOfClass(String oclass) {
        if(oclass.equals(RockSampleDomain.TAXICLASS)){
            return Arrays.<ObjectInstance>asList(taxi);
        }
        else if(oclass.equals(RockSampleDomain.LOCATIONCLASS)){
            return new ArrayList<ObjectInstance>(locations);
        }
        else if(oclass.equals(RockSampleDomain.PASSENGERCLASS)){
            return new ArrayList<ObjectInstance>(passengers);
        }
        else if(oclass.equals(RockSampleDomain.WALLCLASS)){
    ...
    public MutableState set(Object variableKey, Object value) {
        ...
        if(key.obName.equals(taxi.name())){
            if(key.obVarKey.equals(VAR_X)){
                ...
                throw new RuntimeException("Unknown variable key in RockSampleState for RoverAgent: " + variableKey);
            }
            return this;
        }
    ...
    public RockSampleState copy() {
        return new RockSampleState(walls,passengers,locations,taxi);
    }
    ...
    public boolean equals(Object object){
        ...
        if (object instanceof RockSampleState) {
            RockSampleState rockSampleState = (RockSampleState) object;
            if(!rockSampleState.taxi.equals(this.taxi)){
                return false;
            }
            for(TaxiPassenger thisP : this.passengers){
                if(!rockSampleState.passengers.contains(thisP)) {
                    return false;
                }
            }
            for(TaxiLocation thisL : this.locations){
                if(!rockSampleState.locations.contains(thisL)) {
                    return false;
                }
            }
            for(RockSampleMapWall thisW : this.walls){
                if(!rockSampleState.walls.contains(thisW)) {
                    return false;
                }
            }
            return true;
    ...
    public List<RockSampleMapWall> touchWalls(){
        this.walls = new ArrayList<RockSampleMapWall>(walls);
        return walls;
    }

    public List<RockSampleMapWall> deepTouchWalls(){
        List<RockSampleMapWall> nws = new ArrayList<RockSampleMapWall>(walls.size());
        for(RockSampleMapWall w : walls){
            nws.add(w.copy());
        }
        walls = nws;
        return walls;
    ...
    public RockSampleMapWall touchWall(int ind){
        RockSampleMapWall n = walls.get(ind).copy();

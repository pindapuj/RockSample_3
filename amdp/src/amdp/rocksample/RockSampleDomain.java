...
//rocksample is placed randomly at start states
...
public class RockSampleDomain implements DomainGenerator{
    ...
    public static final String								TAXICLASS = "rocksample";
...
//    public static final String                              RED = "red";
    ...
    public RockSampleDomain(RewardFunction rf, TerminalFunction tf) {
        this.rf = rf;
        this.tf = tf;
     ...
     * by the fickle passengers intial location preference when picked up, to the final location preference, chosen when the rocksample
     ...
     * For instance, the probability of the agent moving east when selecting north would be specified in the entry transitionDynamics[0][2]
     *This setup is for when the rocksample is not fickle
     * @param transitionDynamics entries indicate the probability of movement in the given direction (second index) for the given action selected (first index).
     */
...
//        Random rand = randomFactory.ingetMapped(0);
        ...
        domain.addStateClass(TAXICLASS, RoverAgent.class).addStateClass(LOCATIONCLASS, TaxiLocation.class)
            ...
            if(actionInd<4){
                ...
                for(int i = 0; i < directionProbs.length; i++) {
                    ...
                    if (fickleTaxi) {
                        ...
                        List<ObjectInstance> passengers = ((RockSampleState)ns).objectsOfClass(PASSENGERCLASS);
...
//                                System.out.println("passenger was just picked up!");
                                ...
                                int locationIndex = ((RockSampleState) ns).locationIndWithColour(((TaxiPassenger)pass).goalLocation);
                                if(locationIndex==-1){
            ...
            if(actionInd==4){
                ...
                RockSampleState ns = (RockSampleState)s.copy();
                RoverAgent taxi = ns.touchTaxi();
                int tx = taxi.x;
                int ty = taxi.y;
                boolean taxiOccupied = taxi.taxiOccupied;

                if(!taxiOccupied){
                    List<ObjectInstance> passengers = ((RockSampleState)s).objectsOfClass(PASSENGERCLASS);
        ...
        private boolean passengerMoved(State s, State ns, TaxiPassenger pass) {
            TaxiPassenger pOld = (TaxiPassenger)((RockSampleState)s).object(pass.name());
            TaxiPassenger pNew = (TaxiPassenger)((RockSampleState)ns).object(pass.name());
        ...
        protected State move(State sIn, int dx, int dy){
            ...
            RockSampleState ts = (RockSampleState)sIn;
            ...
            if(dx > 0){
                ...
                for(ObjectInstance wall : vwalls){
                    if(wallEast(tx, ty, (RockSampleMapWall) wall) && ((RockSampleMapWall)wall).verticalWall){
                        nx = tx;
                        break;
                    }
                }
                ...
                List<ObjectInstance> vwalls = ts.objectsOfClass(WALLCLASS);
                for(ObjectInstance wall : vwalls){
                    if(wallWest(tx, ty, (RockSampleMapWall)wall) && ((RockSampleMapWall)wall).verticalWall){
                        nx = tx;
                        break;
                    }
                }
                ...
                for(ObjectInstance wall : hwalls){
                    if(wallNorth(tx, ty, (RockSampleMapWall)wall) && !((RockSampleMapWall)wall).verticalWall){
                        ny = ty;
                        break;
                    }
                }
                ...
                List<ObjectInstance> hwalls = ts.objectsOfClass(WALLCLASS);
                for(ObjectInstance wall : hwalls){
                    if(wallSouth(tx, ty, (RockSampleMapWall)wall) && !((RockSampleMapWall)wall).verticalWall){
                        ny = ty;
                        break;
                    }
                }
...
//        }
    ...
    //propositional function for rocksample at location for navigate
...
//            System.out.println(params[0]);
            TaxiLocation location = ns.touchLocation(i);//TaxiLocation)((RockSampleState)s).object(params[0]);
    ...
    public class PF_PickUp extends PropositionalFunction{
        ...
        @Override
        public boolean isTrue(OOState s, String[] params) {
            RoverAgent roverAgent = (RoverAgent)s.objectsOfClass(TAXICLASS).get(0);
            int xt = roverAgent.x;
            int yt = roverAgent.y;
            boolean taxiOccupied = roverAgent.taxiOccupied;
    ...
    protected static boolean wallEast(int tx, int ty, RockSampleMapWall wall){
    ...
    protected static boolean wallWest(int tx, int ty, RockSampleMapWall wall){
        int wallo = wall.wallOffset;
        if(wallo == tx){
            int wallmin = wall.wallMin;
            int wallmax = wall.wallMax;
    ...
    protected static boolean wallNorth(int tx, int ty, RockSampleMapWall wall){
        int wallo = wall.wallOffset;
        if(wallo == ty+1){
            int wallmin = wall.wallMin;
            int wallmax = wall.wallMax;
    ...
    protected static boolean wallSouth(int tx, int ty, RockSampleMapWall wall){
        int wallo = wall.wallOffset;
        if(wallo == ty){
            int wallmin = wall.wallMin;
            int wallmax = wall.wallMax;
    ...
    public static State getClassicState(Domain domain){
        ...
        RoverAgent roverAgent = new RoverAgent(TAXICLASS+0,0,3);
        ...
        List<RockSampleMapWall> walls = new ArrayList<RockSampleMapWall>();
        walls.add(h1);
        walls.add(h2);
        walls.add(v1);
        walls.add(v2);
...
//        taxiPassengers.add(p2);
    ...
    public static State getRandomClassicState(Random rand, Domain domain){
        ...
        RoverAgent roverAgent = new RoverAgent(TAXICLASS+0, rand.nextInt(maxX),rand.nextInt(maxY));
    ...
    public static State getStartStateFromTaxiPosition(int TaxiX, int TaxiY, Random rand, Domain domain){
        ...
        RoverAgent roverAgent = new RoverAgent(TAXICLASS+0, TaxiX, TaxiY);
...
//        State s = RockSampleDomain.getComplexState(false);
...
//                new ConstantValueFunction(0.), new ConstantValueFunction(1.),0.1,-1);
        ...
        State startState1 = RockSampleDomain.getRandomClassicState(rand, td);
        Policy policy = planner.planFromState(startState1);
        Episode episode = PolicyUtils.rollout(policy, startState1, td.getModel());

        Visualizer vis = RockSampleVisualizer.getVisualizer(5, 5);
...
//            td.getModel();
            ...
            for(int i=0;i<1;i++) {
                ...
                for(int j=1;j<ea.stateSequence.size();j++){
                    RockSampleState oldState = (RockSampleState) ea.stateSequence.get(j-1);
                    RockSampleState currentState = (RockSampleState) ea.stateSequence.get(j);
...
//            ObjectInstance location = ((RockSampleState)startState).locations.get(3);
//            System.out.println(((TaxiLocation)location).colour);
                    for(ObjectInstance location:((RockSampleState)startState).locations ){
...
//                        new EpisodeSequenceVisualizer(v, td, Arrays.asList(ea));
            ...
            for(ObjectInstance locationStart:((RockSampleState)exampleStartState).locations ){

                State startState = getStartStateFromTaxiPosition(((TaxiLocation)locationStart).x,((TaxiLocation)locationStart).y, rand, td);
//            ObjectInstance location = ((RockSampleState)startState).locations.get(3);
//            System.out.println(((TaxiLocation)location).colour);
                for(ObjectInstance endLocation:((RockSampleState)startState).locations ){
...
//
                for (int j = 1; j <= numberOfLearningEpisodes; j++) {
                    ...
                    System.out.println("-------------------------------------------------------------");
                    State sNew = RockSampleDomain.getRandomClassicState(rand, td);
